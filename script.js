...
document.addEventListener('DOMContentLoaded', function() {
    ...
    posts.forEach((post, index) => {
        post.addEventListener('click', function() {
            ...
            if (type === 'project') {
                ...
                captionIndex = Math.floor(Math.random() * postCaptions.certificate.length);
    ...
    const followButtons = document.querySelectorAll('.btn-primary, .modal-follow');
    ...
    stats.forEach(stat => {
        ...
        const finalNumber = parseInt(number.textContent.replace('k', '000').replace('.', ''));
    ...
    const animatedElements = document.querySelectorAll('.post-item, .highlight-item');
